...
| 23 | **Round 23 — Backlog polish** | Done. Seis itens MINOR/NIT adiados nos Rounds 21–22 fechados: `PoolConfig.keepaliveTime` exposto via `cdc.outbox.pool.keepalive-time`; `JsonHelper` movido de `core` pra `:legacy` (único chamador) — `core` não expõe mais `org.json`; checagem de anônimo nos endpoints de replay/DLQ trocada de `javaClass.simpleName == "..."` pra `is AnonymousAuthenticationToken` (type-safe), com um 6º caso de teste (`ROLE_GUEST`) fechando o disjunct que a mutation testing do Round 22 achou sub-exercitado; `spring-security-test:7.0.5` não-usado removido de `dlq-replay`; ordem das linhas do Roadmap corrigida (12/19 tinham ido parar no fim da tabela). Validado com build completo + as 5 suítes Testcontainers reais. Detalhe completo em `docs/HISTORY.md`. | Backlog polish — done |
