...
### Observabilidade
  ...
  * **Micrometer** via
    ...
    `cdc.outbox.checkpoint.misconfigured` (Round 26 — incrementa
    quando `cdc.outbox.checkpoint.enabled=true` mas nenhum
    `CheckpointStore` foi wired, tipicamente `cdc-outbox-checkpoint-file`
    faltando; para consumidores MySQL binlog isso é sinal de perda de
    evento a cada restart, vale alerta). Timer:
    `cdc.outbox.publish.duration{sink}`. Gauge:
...
Os blocos principais:
  ...
  * `cdc.outbox.pool.*` — HikariCP (maximumPoolSize, minimumIdle,
    ...
    keepaliveTime — desligado por default, autoCommit — ligado por default).
  ...
  * `cdc.outbox.checkpoint.*` — `enabled` (default `false`) + `directory`.
    Wave 5.2. Liga o `FileCheckpointStore` default **se
    `cdc-outbox-checkpoint-file` estiver no classpath** (Round 26: virou
    `@ConditionalOnClass` — sem o módulo, `enabled=true` não sobe o
    store, só WARN + counter `cdc.outbox.checkpoint.misconfigured`, não
    quebra o boot). Adapters row-level (MySQL binlog,
    `PgWalRowChangeSource`) consomem em `open()` / `ack()`. A varredura
    de `.tmp` órfãos (deixados por crash mid-save) roda no construtor e
    publica `cdc.outbox.checkpoint.orphans_swept`.
...
— **desde que `cdc-outbox-checkpoint-file` esteja no classpath**; sem
o módulo, o boot não quebra, mas nenhum store é instalado (WARN +
counter `cdc.outbox.checkpoint.misconfigured`) e o binlog volta a
reiniciar do head a cada restart, exatamente o comportamento que este
parágrafo descreve evitar.
...
| 26 | **Round 26 — checkpoint-store isolation fail-open fix + root LocalStack pin + jvmToolchain** | Done. Follow-up de 2 itens de um backlog pós-Round-25: (1) `docker-compose.yml` raiz — `localstack:latest` → `3.8.1` (mesmo fix do sample), `version:` obsoleto removido. (2) `cdcOutboxCheckpointStore` isolado num `FileCheckpointStoreConfiguration` aninhado (`@ConditionalOnClass(FileCheckpointStore::class)`) — mesma família de bug do Round 25 (`:checkpoint-file` é `compileOnly`), achada pelo Tech Lead no Round 25 mas fora de escopo lá. Primeira tentativa trocou um crash alto (`NoClassDefFoundError`) por um silêncio: `checkpoint.enabled=true` sem o módulo simplesmente não subia o store — pra `MySqlBinlogRowChangeSource` isso é perda de evento a cada restart, sem log nem métrica. Tech Lead pegou (MAJOR) — corrigido com `cdcOutboxCheckpointMisconfigurationGuard` (novo bean, WARN + counter `cdc.outbox.checkpoint.misconfigured` via `CdcOutboxMetrics`), checado via `ObjectProvider.stream().count()` (não `.ifAvailable`, que lançaria `NoUniqueBeanDefinitionException` com 2+ `CheckpointStore` não-`@Primary`). Segunda revisão do Tech Lead: faltava teste do caminho feliz (módulo presente → store realmente sobe) e um `object` marcador sem sentido como retorno do bean — trocado por `CheckpointConfigurationCheck(val misconfigured: Boolean)`, um valor real. `build.gradle.kts` ganhou `jvmToolchain(21)` (terceira vez que revisões pediam isso — não resolve o problema documentado no `CONTRIBUTING.md` de o parser Kotlin DSL do Gradle 8.10.2 não ler `java -version` no JDK 24+, mas torna a compilação reproduzível uma vez o daemon já de pé). README corrigido em 3 pontos que afirmavam `checkpoint.enabled=true` instala o store incondicionalmente. `./gradlew clean build` sem exclusões + `RUN_TESTCONTAINERS=1 ./gradlew test` verdes (259 testes, 0 falhas, 0 skipped). Tech Lead: FAIL → fix → FAIL → fix → PASS. Detalhe completo em `docs/HISTORY.md`. | Checkpoint fail-open fix — done |
...
|---------------------------|--------------------------------------------------------|
| Kotlin                    | 2.3.21                                                 |
| JVM target                | 21 (Spring Boot 4's próprio baseline mínimo)           |
| Gradle wrapper            | 8.10.2                                                 |
| Spring Boot (compileOnly) | 4.0.6                                                  |
| Spring Framework          | 7.0.7                                                  |
| Spring Kafka              | 4.0.5                                                  |
| Spring AMQP (RabbitMQ)    | 4.0.3                                                  |
...
para o fluxo de fork/PR, estilo de código e as invariantes de entrega
que todo PR precisa preservar. Ao participar, você concorda com o
[Código de Conduta](CODE_OF_CONDUCT.md). Para reportar vulnerabilidades
de segurança, veja [`SECURITY.md`](SECURITY.md) em vez de abrir uma
issue pública.

## Licença

[MIT](LICENSE) © Fernando Fernandes.
